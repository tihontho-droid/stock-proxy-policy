...
prepare_bottom_date = pd.to_datetime(
    selected_prepare_date
)
...
if wave_tops.empty:
    ...
    st.warning(
        "Không có đỉnh ZigZag tăng >= 30%."
    )

    st.stop()

wave_top = wave_tops.iloc[0]
...
if not confirm_top_rows.empty:

    confirm_top_date = (
        confirm_top_rows.iloc[0]["date"]
    )
